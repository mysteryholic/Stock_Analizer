...
def create_candlestick_chart(
    ...
    """캔들스틱 메인 차트 (이동평균선 + 볼린저 밴드 포함, 보조지표 별도 함수)."""
    ...
    fig.update_layout(**_get_chart_layout(height=500))
    fig.update_xaxes(rangeslider_visible=False)
    return fig


def create_volume_chart(df: pd.DataFrame) -> go.Figure:
    """거래량 단독 차트 (메인 차트와 같은 x축 범위)."""
    fig = go.Figure()
    ...
    layout = _get_chart_layout(height=220)
    layout["margin"] = dict(l=50, r=20, t=20, b=30)
    fig.update_layout(**layout)
    return fig


def create_rsi_chart(df: pd.DataFrame, period: int = 14) -> go.Figure:
    """RSI 단독 차트 (30/70 기준선 + 과매수·과매도 영역)."""
    rsi = calculate_rsi(df, period)
    fig = go.Figure()

    fig.add_hrect(y0=70, y1=100, fillcolor="rgba(255,71,87,0.06)", line_width=0)
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(width=1.8, color=COLORS["accent_purple"]),
        fill="tozeroy", fillcolor="rgba(139,92,246,0.08)",
        showlegend=False,
    ))

    layout = _get_chart_layout(height=240)
    layout["margin"] = dict(l=50, r=20, t=20, b=30)
...
def create_macd_chart(df: pd.DataFrame) -> go.Figure:
    """MACD 단독 차트 (MACD/Signal 라인 + 히스토그램)."""
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(width=1.8, color=COLORS["chart_line_1"]),
    ))
    fig.add_trace(go.Scatter(
        x=macd.index, y=macd["Signal"], name="Signal",
        line=dict(width=1.8, color=COLORS["chart_line_2"]),
    ))
    fig.add_hline(y=0, line_color=COLORS["text_muted"], opacity=0.3)

    layout = _get_chart_layout(height=260)
    layout["margin"] = dict(l=50, r=20, t=20, b=30)
    fig.update_layout(**layout)
    return fig
